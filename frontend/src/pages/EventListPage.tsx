...
export default function EventListPage() {
  ...
  const loadEvents = useCallback(async (onlyUpcoming = showUpcomingOnly) => {
    setLoading(true);
    setError(null);
    try {
      const data = await fetchEvents(onlyUpcoming ? { upcoming: true } : undefined);
      setEvents(data);
    } catch (err) {
  ...
  useEffect(() => {
    void loadEvents(showUpcomingOnly);
  }, [loadEvents, showUpcomingOnly]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 text-sm text-slate-600">
          ...
          <button onClick={() => void loadEvents()} className="rounded bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-slate-800">
          Rafraîchir
        </button>
        </div>
      </div>
